...
func switch_scene(next_scene: PackedScene):
	await tween_alpha(hide_alpha)
	await get_tree().create_timer(default_inbetween_duration).timeout
	get_tree().change_scene_to_packed(next_scene)
	await tween_alpha(show_alpha)
