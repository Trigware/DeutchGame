...
func switch_scene(next_scene, visibility_change_duration = default_visibility_diff, in_between_wait_duration = default_inbetween_duration, scene_modifier = null):
	await tween_alpha(hide_alpha, visibility_change_duration)
	var switched_scene = next_scene if next_scene is Node else next_scene.instantiate()
