...
func update_slot():
	...
	var actual_slot_combined_height = item_actual_size * number_of_slots
	var unoccupied_height = window_size.y - actual_slot_combined_height.y
	...
	scale *= item_slot_multiplier
	
	var item_count = get_item_count()
	count_label.text = str(item_count)
	main_sprite.modulate = GridState.team_modulate[item_slot_team]
